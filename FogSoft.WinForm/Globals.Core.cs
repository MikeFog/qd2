	...
	public static partial class Globals
	{
			...
			Dictionary<string, object> filterValues, string filterXml)
		{
			...
			foreach(XmlNode node in xmlDoc.SelectNodes("//*[@value]"))
			{
				...
				filterValues[name] = ResolveInitialValue(value);
			}
		}

		/// <summary>
		/// Значение по сокращению из метаданных фильтра (<see cref="PageControl.InitialValueAbbreviations"/>):
		/// дата относительно сегодня или текущий пользователь; всё прочее — само значение.
		/// Вынесено из <see cref="ResolveFilterInitialValues"/>, чтобы веб мог разобрать то же
		/// сокращение, заданное не в метаданных, а на пункте меню (JournalRoute.FilterDefault).
		/// </summary>
		public static object ResolveInitialValue(string value)
		{
			...
			switch(value)
			{
				case PageControl.InitialValueAbbreviations.PREV_MONTH_BEGIN:
					...
					return currentMonthStart.AddMonths(-1).ToString();
				case PageControl.InitialValueAbbreviations.PREV_MONTH_END:
					currentMonthStart = new DateTime(DateTime.Today.Year, DateTime.Today.Month, 1);
					return currentMonthStart.AddDays(-1).ToString();
				case PageControl.InitialValueAbbreviations.LAST_MONTH:
					return DateTime.Today.AddMonths(-1).ToString();
				case PageControl.InitialValueAbbreviations.LAST_WEEK:
					return DateTime.Today.AddDays(-7).ToString();
				case PageControl.InitialValueAbbreviations.TODAY:
					return DateTime.Today.ToString();
				case PageControl.InitialValueAbbreviations.StartOfTheMonth:
					return new DateTime(DateTime.Today.Year, DateTime.Today.Month, 1).ToString();
				case PageControl.InitialValueAbbreviations.StartOfTheLastMonth:
					return new DateTime(DateTime.Today.Year, DateTime.Today.Month, 1).AddMonths(-1).ToString();
				case PageControl.InitialValueAbbreviations.EndOfTheMonth:
					return new DateTime(DateTime.Today.Year, DateTime.Today.Month, 1).AddMonths(1).AddDays(-1).ToString();
				case PageControl.InitialValueAbbreviations.LoggedUser:
					return SecurityManager.LoggedUser.Id;
				default:
					return value;
			}
		}

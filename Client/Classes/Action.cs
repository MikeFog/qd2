...
{
    // UI-часть — в Action.WinForms.cs: DoAction, PrintMediaPlan (делегирует в
    // MediaPlan.Show, отдельная отложенная область — §6), GetBill/CreateBill,
    // ChangeFirm/ChangeCreator, вся генерация документов (PrintContracts и
    // семейство — отдельная область, этап 4), SetAdvertTypeOrSubstituteRoller.
    ...
    public abstract partial class Action : ObjectContainer
    {
        ...
        // PrintAgencyDocument (delegate) переехал в Action.WinForms.cs.
        ...
        // PrintContracts/PrintBillContracts/PrintSponsorContracts/PrintBills переехали в Action.WinForms.cs.
        ...
        // PrintContract/PrintSponsorContract/PrintBillContract/PrintBill переехали в Action.WinForms.cs.
        ...
        // SetAdvertTypeOrSubstituteRoller переехал в Action.WinForms.cs (Globals.ShowSimpleJournal, Cursor).
        ...
        private static int GetSlotIssueCount(
            IssueSlotKey slot,
            List<Dictionary<IssueSlotKey, List<DataRow>>> groupedIssues)
        {
        ...
        private bool CreateAddedIssueRow(DataTable target, DataRow source, DateTime issueDate)
        {
            ...
            newRow[Issue.ParamNames.PositionName] = Issue.GetPositionDisplayName((RollerPositions)positionId);
        ...
        {
            if (ColumnExists(row, Issue.ParamNames.PositionId))
                return ParseHelper.ParseToInt32(row[Issue.ParamNames.PositionId].ToString(), (int)RollerPositions.Undefined);

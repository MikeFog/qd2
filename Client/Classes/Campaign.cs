...
namespace Merlin.Classes
{
    internal class Campaign : CampaignPart
    {
		...
		public bool DeleteIssues(IWin32Window owner, bool isSponsorProgram = false, Dictionary<string, object> extraParameters = null, bool isFireEvent = true)
		{
            ...
            try
			{
				...
				if (selector.ShowDialog(owner) == DialogResult.OK)
				{
					...
					{
						Globals.ShowSimpleJournal(EntityManager.GetEntity((int)Entities.ErrTmplGen), "������ ��������", tableErrors);
					}
        ...
        public void PrintMediaPlan(bool isActual, bool isByMonth, bool isByPeriod, bool selectively)
		{
			...
			{
				FrmDateSelector selector = new FrmDateSelector(StartDate, FinishDate, "����� �������");
		...
		private void ChangePaymentType(IWin32Window owner)
		{
			if (IsChangePossible)
			{
				SelectionForm selector = new SelectionForm(EntityManager.GetEntity((int)Entities.PaymentType), "���� ������");
				if (selector.ShowDialog(owner) == DialogResult.OK)
				{
		...
		internal void DisplayCampaignData(ListBox lstStat)
		{
			...
			lstStat.Items.Add("���������: " + text);
			lstStat.Items.Add("��������: " + IssuesCount);
			lstStat.Items.Add("����� �����: " + DateTimeUtils.Time2String(IssuesDuration));
			lstStat.Items.Add("���� �� �������: " + TariffPrice.ToString("c"));
			lstStat.Items.Add("�������� ������: " + Discount.ToString("0.00"));
			lstStat.Items.Add("���� � ������ �������� ������: " + Price.ToString("c"));

			if (CampaignType == CampaignTypes.Sponsor)
			{
				lstStat.Items.Add("");
				lstStat.Items.Add("��������: " + ProgramIssuesCount);
				lstStat.Items.Add("�����: " + DateTimeUtils.Time2String(Bonus - IssuesDuration));
			}
		}

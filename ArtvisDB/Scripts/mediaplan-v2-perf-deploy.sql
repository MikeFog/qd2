/*
    ...
    ЦЕНА
      ...
      кампания в 10 выпусков). За один экспорт медиаплана процедура зовётся
      по разу на станцию (в замере — 15 раз) — ~375 мс компиляции на весь
      экспорт. Всё равно кратный выигрыш в любом сценарии.
...
BEGIN
    ...
    --
    --   Цена: ~25 мс компиляции на вызов (замер на ArtvisDev). За один экспорт
    --   медиаплана процедура зовётся по разу на станцию (в замере — 15 вызовов),
    --   т.е. ~375 мс компиляции на весь экспорт. Всё равно кратный выигрыш:
    --   даже кампания в 10 выпусков стоила 846 мс, стала ~25 мс.
    --------------------------------------------------------------------
    IF @isFact = 1
    BEGIN
...
GO
-- В SSMS выполнение после сработавшего guard'а доходит сюда (SET NOEXEC ON
-- пропускает только батчи, PRINT под ним не выполняется — но эти строки идут
-- уже после SET NOEXEC OFF). Поэтому не пишем «Готово» безусловно: сверяем
-- recompile_в_теле фактически.
IF (SELECT (LEN(m.definition) - LEN(REPLACE(m.definition, 'OPTION (RECOMPILE)', ''))) / LEN('OPTION (RECOMPILE)')
    FROM sys.sql_modules m WHERE m.object_id = OBJECT_ID('dbo.MediaPlanRetrieve_v2')) = 2
    PRINT 'Готово. recompile_в_теле=2 — фикс на месте.';
ELSE
    PRINT 'ВНИМАНИЕ: recompile_в_теле <> 2 — ALTER не применился (сработал guard?). Проверьте вывод выше.';

-- Клонирование прайс-листа: перенос всех атрибутов тарифа.
--
-- 1) PricelistIUD (@actionName = 'Clone', в т.ч. массовое клонирование): в клон тарифа теперь переносятся
--    тип блока (blockTypeID) и флаги DJin notEarly / notLater / openBlock / openPhonogram - раньше они
--    терялись (после клона: тип блока пуст, флаги выключены).
-- 2) sponsorPLIUD (@actionName = 'Clone', спонсорский прайс-лист): в клон SponsorTariff теперь переносятся
--    isAlive («прямой эфир»), path, suffix, needExt, needInJingle, needOutJingle - раньше подставлялись
--    значения по умолчанию.
--
-- Тарифы, склонированные ДО этого исправления, автоматически не восстанавливаются.
-- Скрипт идемпотентен (CREATE OR ALTER), таблицы и данные не трогает. Права на процедуры сохраняются.
--
--   sqlcmd -S <сервер> -d <база> -E -f 65001 -I -i tariff-clone-full-attributes-deploy.sql
...
IF @actionName = 'Clone' AND ISNULL(@cloneMode, 0) NOT IN (1, 2, 3) BEGIN
...
PRINT '--- Проверка: определения процедур содержат новые колонки ---';
SELECT 'PricelistIUD (blockTypeID/notEarly/notLater/openBlock/openPhonogram)' AS [объект],
       CASE WHEN OBJECT_DEFINITION(OBJECT_ID('dbo.PricelistIUD')) LIKE '%src.openPhonogram%' THEN 'OK' ELSE 'НЕТ' END AS [состояние]
UNION ALL
SELECT 'sponsorPLIUD (isAlive/path/suffix/needExt/needInJingle/needOutJingle)',
       CASE WHEN OBJECT_DEFINITION(OBJECT_ID('dbo.sponsorPLIUD')) LIKE '%[[]isAlive], [[]path]%' THEN 'OK' ELSE 'НЕТ' END;
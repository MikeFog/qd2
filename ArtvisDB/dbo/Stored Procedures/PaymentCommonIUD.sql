...
CREATE PROCEDURE [dbo].[PaymentCommonIUD]
(
@paymentID int = NULL,
...
IF @actionName = 'AddItem' BEGIN
	INSERT INTO [Payment](firmID, summa, paymentDate, userID, paymentTypeID, agencyID, isEnabled)
	...
	EXEC PaymentsCommon @paymentID = @paymentID, @loggedUserID = @loggedUserID
	END
...
ELSE IF @actionName = 'UpdateItem' BEGIN
	...
	IF EXISTS(SELECT * FROM PaymentAction WHERE paymentID = @paymentID) BEGIN

CREATE PROCEDURE [dbo].[FirmManagers]
(
	@firmID int,
...
BEGIN
	...
	WHERE 
		...
		and u.userID <> 0	
		and (u.userID in (select a.userID from [Action] a where a.firmID = @firmID)
			or u.userID in (select p.userID from Payment p where p.firmID = @firmID))

...
## 4а. Доработка «дни недели в интервалах» (24.08.2026)

Отдельная, более поздняя доработка: строка интервалов-исключений может
начинаться с дней недели (`пн-пт 16:00-16:55; 18:00-19:00`, на следующей строке
другой набор). Описание — в `political-agitation-ads.md`. Схема **не** меняется,
старые значения (без дней) продолжают работать как «все дни».

Если основной функционал уже развёрнут, нужно только это:

```
sqlcmd -S <сервер> -d <база> -E -f 65001 -I -i ArtvisDB\Scripts\political-agitation-interval-days.sql
```

Скрипт обновляет текст сообщения `AgitationIntervalsInvalid` и подпись поля на
вкладке паспорта; он идемпотентен и в конце печатает таблицу проверки.

Затем задеплоить (`ALTER`) два объекта — командой из шага 2, сократив список
`$files` до них:

| Файл | Что изменилось |
|---|---|
| `dbo/Functions/fn_AgitationExcludeIntervals.sql` | разбор дней недели, колонка `dayMask` |
| `dbo/Stored Procedures/AgitationFraming.sql` | сравнение дня недели окна с `dayMask` |

`MassmediaIUD` править не нужно: валидация опирается на `startMin IS NULL`, а
функция возвращает NULL и для неверно заданных дней.

Клиент пересобирать **не** нужно — правок C# в доработке нет, но **перезапустить
его надо**: подпись поля приходит из кэшируемых метаданных паспорта.

Проверка после развёртывания: сохранить в карточке станции строку
`пн-пт 17:00-18:00`, разместить агитацию в понедельник в 17:xx (должен появиться
только анонс) и в субботу в 17:xx (должна появиться полная обвязка).

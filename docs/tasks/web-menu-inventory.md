...
| 64 | Трафик → Сетка вещания | `miPrintGrid` | `:196` → ShowPrintGridForm:670 | Отчёт / выгрузка | `FrmGridReport` (Crystal-просмотрщик) | перенесён (`/broadcast-grid`, 2026-09-25; «Для эфира…» — файл DJin) | 2 | — | 5 |
| 152 | Трафик → Экспорт сеток вещания | `miExportGrid` | `:234` → ExportGrid:309 | Отчёт / выгрузка | `ExportGridForm` (файлы на диск) | перенесён (`/grid-export`, 2026-09-25; выбор папки в браузере, Word — docx) | 1 | — | 5 |

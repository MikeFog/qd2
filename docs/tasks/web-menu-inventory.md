...
| 62 | Рекламный отдел → Сетка вещания | `miPrintGridFromRSection` | `:196` → ShowPrintGridForm:670 | Отчёт / выгрузка | `FrmGridReport` (Crystal-просмотрщик) | перенесён (`/broadcast-grid`, 2026-09-25; выгрузка для эфира — с «Экспортом сеток») | 6 | +4/−0 | 24 |
...
| 64 | Трафик → Сетка вещания | `miPrintGrid` | `:196` → ShowPrintGridForm:670 | Отчёт / выгрузка | `FrmGridReport` (Crystal-просмотрщик) | перенесён (`/broadcast-grid`, 2026-09-25; выгрузка для эфира — с «Экспортом сеток») | 2 | — | 5 |
...
**Уточнено 2026-09-23 (решение владельца): диаграммы отложены для всей
статистики, не только для этого пункта.** Сейчас делаем базу; диаграммы —
позже и одним общим подходом для всех журналов статистики, а не точечно под
`GraphForm`. Что учесть, когда дойдёт:

- в десктопе на деле только «Список» и «Пирог» по `sum1` (столбцы в
  `ReportGenerator.ReportTypes` закомментированы), пирог — лишь при одной
  разбивке (`VolumeOfRealizationReportGenerator.ReportTypes`);
- повтор один в один бессмыслен: при разбивке по фирмам за неделю ~170 строк,
  основная масса — доли 0,1–0,5 %, пирог нечитаем. Нужен продуманный вид
  (первые N + «прочие», горизонтальные столбцы по убыванию или иное) и правило,
  какие колонки и журналы им показываются;
- библиотеки графиков в вебе нет; выбор (свой SVG или внешняя) — часть того же
  решения.

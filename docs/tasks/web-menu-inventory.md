...
| Дерево на своём контейнере | 6 | нет: 5 журналов акций + «Предмет рекламы» |
| `MasterDetail` | 4 | нет, движка нет |
| Журнал-наследник со своей логикой | 4 | нет (`miRoller`, `miActPrint`, `miStats.Balance`, `miAnnouncements`) |
| Собственная форма | 6 | нет, этап 3 |
| Диалог → журнал(ы) | 1 | нет (`miPaymentByManager`) |
| Мастер: диалоги → карточка акции (новая) | 3 | нет, этап 3 |
| Действие без экрана | 3 | нет, удаляют данные |
| Импорт из файла (новая) | 1 | нет, этап 4 (`miFirmImport`) |
| Отчёт / выгрузка | 4 | нет, этап 4 |
| График (новая) | 1 | нет (`VolumeOfRealizationByManager`) |
| Выход из приложения (новая) | 1 | пункт ведёт на заглушку, есть кнопка «Выйти» |
...
| Статус | Пунктов |
|---|---:|
| перенесён (29 `SimpleJournal` + 7 `Browser`) | 36 |
| почти одной строкой (§5) | 3 |
...
| не нужен / не экран (`miExit`, `miAccounting`) | 2 |
| **Итого** | **71** |
...
**Вывод: список — почти одной строкой** (сущность 179 без правил), но «Добавить»
и удаление в вебе включатся по правам сущности (`Journal.razor:157`) — в
десктопе они спрятаны безусловно; «пометить прочитанным» — действие по строке
(`Announcement.cs:39`, `SetReadMark` приватный, `DoAction` в UI-половине
`Announcement.WinForms.cs:14`), для него нужен путь выполнения действия по
строке, которого в вебе пока нет.
...
1. **`miStats.Balance`** — маршрут в `MenuRoutes.SimpleJournal` плюс подмена
...
2. **`miAnnouncements`** — маршрут на сущность 179 даёт список (проверить не
   запускали). Что не даст: скрытие «Добавить»/удаления (в десктопе безусловное,
   в вебе — по правам сущности) и «Пометить все как прочитанное» (§4).
   Разумно подключать список сразу, кнопку — вместе с действиями по строке.
3. **`miAdvertSubject`** — запись в `MenuRoutes.Browser` со сценарием
...
4. **`miExit`** — в вебе пункт `miExit` ведёт на заглушку
...
Ctrl+Shift+A. В вебе их нет; нужны ли — не установлено.
...
- `miExit` — заглушка при работающей кнопке «Выйти» (§5).
...
3. **`miPaymentByManager`**: в вебе одно окно — одна вкладка браузера. Оставлять
   ...
   правилом менеджера? Права у пунктов разные (3 и 6 групп).
4. **`VolumeOfRealizationByManager`**: нужны диаграммы (столбцы и круг) или
   хватит таблицы — того же журнала 158 с менеджером по умолчанию?
5. **Автопоказ сообщений** отключён `return;` с 2026-03-17 (`MDIForm.cs:842`).
   ...
   десктопе)?
6. **Три «удаления»** (`deleteDummyRollers`, `deleteDeletedActions`,
   `deleteUnconfirmedActions`): нужны ли в вебе вообще? В десктопе — личная
   выдача двум пользователям на каждый.
7. **Шесть пунктов без выдач** (§7): мёртвые или ещё не розданы? На проде?
8. **Дубли пунктов**: три входа в журнал подтверждённых акций, два — в журнал
   оплат, баланс, сетку вещания, статистику роликов. Права выдаются по пункту,
   поэтому в вебе они остаются раздельными — устраивает ли, или сливать меню?
9. **`miExit`** в вебе: убрать пункт или сделать выходом из сеанса?
10. **Горячие клавиши** (Alt+X, Ctrl+Alt+R, Ctrl+Shift+A): переносить ли?
11. **Заголовок журнала** в вебе — имя сущности, в десктопе — текст пункта
    меню (вопрос уже стоит в `web-migration.md`, подраздел «Журналы статистики»).
...
| 169 | Рекламный отдел → Группа компаний | `miHeadOrganizations` | `:249` → ShowHeadCompanies:610 | MasterDetail | HeadCompany (1248) → Firm (16), `ShowInactive=1` | нужен движок MasterDetail | 7 | — | 28 |
| 39 | Рекламный отдел → Предмет рекламы | `miAdvertSubject` | `:155` → ShowAdvertSubjects:509 | Дерево на своём контейнере | `AdvertTypeContainer` (сценарий «Предметы рекламы»: 17 / 1243) | почти одной строкой (§4) | 7 | +7/−0 | 26 |
...
| 22 | Рекламный отдел → Выход | `miExit` | `:137` → ApplicationExit:985 | Выход из приложения | `Application.Exit()` | не нужен: есть кнопка «Выйти» (`MainLayout.razor:28`); пункт ведёт на заглушку | 7 | +3/−0 | 27 |
...
| 77 | Бухгалтерия → Журнал оплат | `miPaymentCommon` | `:181` → ShowPaymentCommon(false):628 | MasterDetail | PaymentCommon (145) → PaymentCommonAction (146), `filterAgencies=false` | нужен движок MasterDetail | 3 | +1/−0 | 9 |
...
| 92 | Бухгалтерия → Специальные отчёты → Cальдо расчётов по всем фирмам-заказчикам в разрезе агентств | `miStats.Balance` | `:765` → ShowStatBalance:816 | Журнал-наследник со своей логикой | `StatBalanceJournalForm`: StatsBalance (160) / StatsBalanceGroup (185) | почти одной строкой (§4) | 1 | — | 5 |
...
| 75 | Администрация → Агентства и налоги | `miAgencyTax` | `:212` → ShowAgencyTaxJournal:737 | MasterDetail | Agency (8) → AgencyTax (143) | нужен движок MasterDetail | 2 | — | 5 |
...
| 163 | Администрация → Удаление роликов-пустышек | `deleteDummyRollers` | `:238` → DeleteDummyRollers:270 | Действие без экрана | вопрос + `DeleteUnusedDummyRollers` | решение владельца (удаляет данные) | 0 | +2/−0 | 2 |
| 164 | Администрация → Очистка журнала удаленных рекламных акций | `deleteDeletedActions` | `:240` → DeleteDeletedActions:283 | Действие без экрана | вопрос + `DeleteDeletedActions` | решение владельца (удаляет данные) | 0 | +2/−0 | 2 |
| 165 | Администрация → Удаление макетов рекламных акций | `deleteUnconfirmedActions` | `:242` → DeleteUnconfirmedActions:296 | Действие без экрана | вопрос + `DeleteUnconfirmedActions` | решение владельца (удаляет данные) | 0 | +2/−0 | 2 |

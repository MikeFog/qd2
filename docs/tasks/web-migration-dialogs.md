...
7. **И мост тоже не доказательство сам по себе — нужны оба билда.** Мост
   может НЕ показать все ошибки для одного и того же непойманного имени в
   одном файле: если `using System.Windows.Forms` уже дал CS0234, дальнейшие
   обращения к типам оттуда (например, `Application.DoEvents()` вторым
   методом ниже по файлу) мост иногда не репортит вовсе — но десктопная
   сборка после того же изменения ломается по-настоящему (найдено в
   `MassmediaPricelist.cs`, §10). После правки ядра — собирать **и** мост,
   **и** `qd2.sln`, не полагаться на «мост зелёный, значит порядок».
...
### Мост закрыт (2026-08-21, партии 20-22, коммит `cae5912`)

`dotnet build FogSoft.Core/FogSoft.Core.csproj` — **0 ошибок**. Подключены
все домены классы `Client/Classes` (кроме сознательно исключённых, см.
ниже), плюс `Resources.Designer.cs` и пакет `System.Drawing.Common`.
`qd2.sln` пересобран полностью (`Rebuild`, не инкрементально) — тоже 0
ошибок, только два известных, не связанных с этой правкой предупреждения.

Подтверждён вывод предыдущего раздела на практике: доводили не «сначала
вычистим по grep, потом подключим», а «подключаем всё, чиним по указанию
компилятора». Добит длинный хвост похожих мест — `DoAction`/`AssignNew`/
`ShowFilter` с `IWin32Window` в сигнатуре без единого `ShowDialog`,
разбросанные по ~30 файлам, включая базовый `FakeContainer` в
`FogSoft.WinForm` (тот же приём, что `ObjectContainer` на этапе 0.1:
интерфейс `IActionHandler` с UI-типом в контракте объявляется в
`.WinForms.cs`-половине, а не в ядре).

**Важная находка о самом методе проверки:** мост не всегда репортит все
ошибки для одного и того же непойманного имени в файле. В
`MassmediaPricelist.cs` было два метода с `Application.DoEvents()`; когда
`using System.Windows.Forms` уже был отмечен как CS0234, дальнейшие
обращения к `Application` в том же файле мост **не показал вовсе** — хотя
реально ломали десктоп-сборку после того, как using убрали. Поймано только
потому что после каждой правки моста проверяется **и** `qd2.sln`. Если бы
проверялась только сборка моста — регрессия проскочила бы. Чек-лист §7
дополнен явно: проверять оба билда после каждого файла, не полагаться на
то, что зелёный мост = зелёный десктоп.

Три места потребовали не переноса, а точечного решения (не тиражирование):

- **`IssueTransfer.cs`** — `Globals.ShowQuestion(...) == DialogResult.Yes`
  заменён на `UserInteraction.Confirm(MessageAccessor.GetMessage(...))`,
  тот же паттерн, что уже применён к `DeactivateAction` раньше. Правка не
  переносит код, а переписывает его на существующую точку подстановки.
- **`Massmedia.cs`** — `Update()` содержит скрытый UI: лицензионная
  проверка `AdvertAgLicence.CheckLicenseMassmediasCountForAdd()` сама
  показывает `UserMessage`. Модуль лицензий — **живая подсистема**, не
  рудимент (обсуждено в чате 2026-08-21): проверяется при каждом старте
  приложения (`MDIForm.cs`), перепроверяется по таймеру, гейтит
  добавление радиостанций сверх лицензии, и для него есть отдельный
  проект `Protector` для выпуска ключей. Трогать саму подсистему не входило
  в задачу — `Update()` переехал в `Massmedia.WinForms.cs` целиком, без
  малейшего изменения поведения (виртуальный метод, диспетчеризуется
  одинаково независимо от того, в какой из двух частей `partial class`
  лежит реализация).
- **`Utils.cs`** — решение «файл не разрезаем» (см. форма 4, `AskConfirmation`)
  было верным для цели «не переносить в веб», но неверным для цели «мост
  собирается»: `CreateBankById` в том же файле — чистый метод, нужный
  `Organization.cs`, и из-за соседства с `AskConfirmation`/
  `HideTableLayoutRow` он тоже был недоступен ядру. Разрез сделан:
  `CreateBankById` остался в `Utils.cs`, `AskConfirmation`/
  `HideTableLayoutRow` переехали в `Utils.WinForms.cs`. Сама функция
  `AskConfirmation` не изменилась и не переносится в веб — решение
  2026-08-21 не пересмотрено, изменилось только физическое место кода.

Дополнительно закрыт `RollerPartOfSponsorCampaign.cs` — мирный близнец уже
разрезанного `ProgramPartOfSponsorCampaign.cs`, тот же перенос целиком без
разреза (форма 3.1, §8 п.3): по какой-то причине при обходе `ShowDialog` он
не попал в работу, хотя пара к нему была сделана.

**Попутно найдено, но не исправлено (не задача этой правки):**
`MassmediaDiscount.cs`, `FakeContainers/MassmediasContainer.cs`,
`FakeContainers/StudioOrderActionContainer.cs` — три файла с реальным кодом,
которые **не подключены в `Client.csproj`** и потому не участвуют в сборке
десктопа. К ним применён тот же разрез для единообразия (раз уж мост их
всё равно затронул), но раз они не собираются в `Merlin.exe`, эффекта на
приложение это не даёт. Удалять не стали — решение об удалении дохлого
кода не входит в задачу разбора хвостов.

**Сознательно вне моста**, с обоснованием, не техническим долгом:

| Файл | Почему |
|---|---|
| `Launcher.cs` | `[STAThread] Main()` — точка входа, изначально десктопная |
| `MediaPlan.cs` | форма 1/2, разрез малополезен сейчас — см. §9 |
| `Campaign.WinForms.cs` / `ProgramPartOfSponsorCampaign.WinForms.cs` / `RollerPartOfSponsorCampaign.WinForms.cs` (UI-половины) | форма 3.1, модальная сессия `CampaignForm` — отложено до этапа 3 |

Это не хвосты, а осознанные архитектурные границы этапа 0 — на них
натыкаться повторно не нужно, они уже разобраны в §6 и §9.

### Предупреждение тому, кто будет обрезать мост скриптом (актуально навсегда)
...
`Classes.X.cs` совпадает **и** с `..\FogSoft.WinForm\Classes\X.cs`. В этой
же сессии раньше из проекта каскадом вылетели `SecurityManager`,
`ConfigurationUtil`, `EntityManager`. Второй раз мост дорабатывался только
точечными Python-правками по одному файлу/группе файлов с проверкой сборки
после каждой — обрезка целиком больше не применялась.

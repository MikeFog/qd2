...
- тариф — шаблон, окно — экземпляр со своими копиями продолжительности, цены, времени;
  точечные изменения делаются в окнах, расхождение окна с тарифом — норма
  (`docs/business-logic.md`, «Тариф и рекламное окно»). Сетки всегда показывают значения окна;
...
- `TrafficGrid.cs:772–773` (`TransferIssue`): в цикле переноса отмеченных выпусков после
  успешного переноса используется `SelectedIssue` (текущая строка таблицы), а не переменная
  цикла. В базе всё верно (переносит `SimpleTransfer` по переменной цикла), но картинка до
  обновления врёт: счётчик окна-источника уменьшается на длительность ролика текущей строки,
  а не перенесённого, и `grdIssue.DeleteRow(SelectedIssue)` удаляет из таблицы текущую строку —
  после первой итерации её уже нет, дальше удаляется не то или ничего. При переносе
  нескольких выпусков разной длины остаток окна-источника неверен, в таблице остаются
  перенесённые и пропадают неперенесённые, пока не перечитать сетку. Лечится заменой
  `SelectedIssue` на перенесённый выпуск в двух строках — **исправлено 24.09.2026**.

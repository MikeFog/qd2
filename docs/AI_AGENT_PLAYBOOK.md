...
**Отбор своего веб-экрана — тоже метаданными** (решение владельца 2026-09-25: одинаковое
делать одинаково). Не верстать поля отбора разметкой экрана: у экрана с сущностью — её
`iEntity.filter`, без сущности — именованный паспорт в `iPassport`; рисует `FilterPanel`,
...
### Текст в вебе: только через `Tr` (многоязычность)

Веб многоязычный (ru — исходный, es), десктоп — нет. Механизм и решения —
`docs/tasks/web-i18n.md`. Ключ перевода — сам русский текст.

**Правило для нового веб-кода:**

- любой видимый пользователю русский текст — через `Tr.T("…")`; с подстановками —
  `Tr.Format("Создано: {0}", n)`, целое предложение одним ключом, даты — аргументом
  `DisplayFormat.Date(x)`;
- в разметке атрибуты — только в скобках: `title="@(Tr.T("…"))"`;
- перевод — в момент показа: не в `const`, `static readonly` и значениях параметров
  по умолчанию (иначе язык «замерзает» на первом пользователе);
- подписи из метаданных (`Entity.Name`, `Attribute.Alias`, `Action.Alias`, `caption`)
  при выводе тоже через `Tr.T`;
- формат дат и денег для показа — `DisplayFormat` и `ToString("C")`, не `"dd.MM.yyyy"`;
- то, что осознанно не переводится (ключ данных, лог, текст для разработчика), —
  пометка `// i18n-ok: причина` на строке.

Проверка: `python FogSoft.Web/find-untranslated.py` — должно быть «Итого … 0».
Непереведённое на живом экране видно в режиме «[·Псевдо·]» (переключатель языка в
шапке, только Development): всё, что прошло через `Tr`, в рамке.

В общем с десктопом коде (`FogSoft.Core` → `FogSoft.WinForm`, `Client/Classes`)
`Tr.T` тоже можно и нужно использовать: в десктопе переводчик не задан, текст
остаётся прежним.

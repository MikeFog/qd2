...
| 12 | **Перепривязка `DataSource` сохраняет ширины колонок** | `Clear()` пересоздаёт колонки с шириной 100. Поэтому сеттер `DataSource` запоминает ширины и `AutoSizeMode` (`SaveColumnLayouts`) и восстанавливает их (`RestoreColumnLayouts`), **только если набор колонок совпал** (ключ `Name|DataPropertyName|HeaderText`: `Name` у колонок сущности пуст). Если набор другой — ширины по умолчанию, форма сама зовёт `AdjustColumnsWidthExt`. Прямой вызов `Clear()` ширины не сохраняет |

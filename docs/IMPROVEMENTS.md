...
- [Веб: новый объект отправлял в процедуру только тронутые поля](tasks/web-passport-new-object-fields.md) — «Добавить тариф» в вебе не работало никогда (NULL в duration_total); дефект движка карточки, а не тарифа; найдено и исправлено 22.09.2026, остались два вопроса владельцу.
...
### [UI-04] После удаления объекта дерево перечитывается из БД (`RebuildCurrentNode`), хотя узел уже убран

**Область:** `SmartGrid.OnObjectDeleted` / `SmartGrid.DeleteSelectedObjects` (хвост `RunSingleDeleteTail`) / `TreeView2.RebuildTree`
**Суть:** Когда слева дерево, а справа `SmartGrid` с содержимым раскрытого узла, удаление объекта делает подряд два независимых действия над деревом. Первое: `ObjectDeleted` -> `TreeView2.OnGridObjectDeleted` -> `DeleteNode` убирает узел из дерева в памяти (поиск по `PresentationObject.Equals`, то есть по ID и `Entity.Id`) и чистит кэш родителя. Второе: `RebuildCurrentNode` -> `TreeView2.RebuildTree` вызывает `CurrentObject.Refresh()` (запрос к БД), сворачивает узел, очищает детей и кэш контейнера, заново загружает содержимое из БД и разворачивает узел обратно с сохранением раскрытых веток. Получается, что узел удалён дважды: сначала «по-честному», потом перечитыванием всего.
**Почему важно:** Объект уже удалён процедурой в БД, ошибки не пришло — состояние известно, перепроверять его перечитыванием не нужно. Это лишние обращения к БД на каждое удаление, а на проде уже бывают дедлоки (см. память по дедлокам): чем меньше лишних чтений рядом с записью, тем лучше. Предположительная история: `RebuildCurrentNode` когда-то добавили как «перечитать всё из базы, чтобы точно построилось верно», когда не удавалось корректно обновить дерево точечно (владелец кода, 20.09.2026). Единственное, что `RebuildTree` делает сверх `DeleteNode` — перечитывает самого родителя (`CurrentObject.Refresh()`), т. е. обновляет его агрегаты (счётчики, суммы), если они есть; какие именно родители от этого зависят — не проверено.
**Где смотреть:**
- `FogSoft.WinForm\Controls\SmartGrid.cs` — `OnObjectDeleted` (одиночный путь: `DeleteRow` с событием, `RefreshDependantGrid`, `RebuildCurrentNode`), `RunSingleDeleteTail` (массовый путь по флагу `iEntity.isMassDeleteAllowed` повторяет то же самое: `ObjectDeleted` по каждому, затем один раз `RefreshDependantGrid` и `RebuildCurrentNode`)
- `FogSoft.WinForm\Controls\TreeView2.cs` — `DependantGrid` (подписки), `OnGridObjectDeleted`/`DeleteNode`/`FindNode`, `RebuildTree(PresentationObject)`
- аналогичные пары есть и в `OnObjectChanged` (`UpdateRow` + `RefreshDependantGrid` + `RebuildCurrentNode`)

**Возможное направление:** Убрать `RebuildCurrentNode` из пути удаления, оставив точечное `DeleteNode` (плюс `ClearCache` родителя, он уже есть). Если у каких-то родителей агрегаты в дереве действительно зависят от детей — обновлять их точечно и только для этих сущностей (например, флаг или переопределение), а не перечитывать весь узел на каждое удаление. `RefreshDependantGrid` тоже перечитывает содержимое из БД и тоже стоит проверить: в гриде строка уже удалена через `DeleteRow`. Перед изменением проверить на форме «дерево + грид» с раскрытым узлом: одиночное и массовое удаление, состав узлов, счётчики родителя, раскрытость веток. Меняет общий код всех ExplorerForm/MasterDetailForm, поэтому — отдельной задачей.
...
### [ENTITY-01] `EntityCampaign.Equals` считает равными пять разных сущностей кампаний

**Область:** `Client\Classes\EntityCampaign.cs`, `FogSoft.WinForm\Controls\SmartGrid.cs` (`IsAllowedEntity`)
**Суть:** `Equals` переопределён так, что любая сущность-кампания (78, 91, 92, 93; 171 тоже `EntityCampaign`, но в списке id её нет) равна любой другой. `GetHashCode` остался базовым, так что контракт нарушен: «равные» объекты дают разные хеши. Сравнение несимметричное, потому что работает, только когда слева стоит `EntityCampaign`.
**Кто на это опирается:** дерево (`TreeView2.RebuildTree`) больше не опирается: с 2026-09-26 оно ищет предка по условию `po => po is Campaign`. Остаётся `SmartGrid.IsAllowedEntity` (`entityCandidate.Equals(entity)`): удалённая кампания одного типа убирается из грида, настроенного на сущность другого типа (например, объект 91 в гриде на 78 в `ActionForm`). Если снять переопределение, в таких гридах строка после удаления останется.
**Почему важно:** это мина для любого будущего кода, который сравнивает сущности или кладёт их в `Dictionary`/`HashSet`.
**Возможное направление:** в `IsAllowedEntity` явно проверять «та же семья сущностей» (например, кампания ↔ кампания через `presentationObject is Campaign`-подобный признак, передаваемый гриду), после чего удалить `EntityCampaign.Equals`/`GetHashCode`, а если класс опустеет — и сам `EntityCampaign` вместе со строками `iEntity.entityClassName`.
...
### [SQL-05] Наборы скидок радиостанции (`DiscountRelease`): «открытый» конец периода — единственный такой случай, и `AddItem` ломает историю

**Область:** `dbo.DiscountRelease`, `DiscountReleaseIUD` (`AddItem`, `UpdateItem`), `DiscountReleases`, `hlp_CompanyDiscountCalculate`
**Суть:** в отличие от всех остальных периодных сущностей (прайс-листы, пакетные скидки `PackageDiscountPriceList`, спонсорские прайсы и т.д. — там явные `startDate` + `finishDate` и проверка «периоды не пересекаются»), у набора скидок радиостанции конец периода **не вводится**: `finishDate` — производное значение, его выставляет процедура: последний набор имеет `finishDate IS NULL` (открыт), а добавление следующего «закрывает» предыдущий. На ArtvisDev 43 набора из 45 открыты (`finishDate IS NULL`) — по одному на радиостанцию, что нормально для схемы.
**Дефект (баг, а не только улучшение).** Скидка ищется по `startDate <= дата < finishDate` (`hlp_CompanyDiscountCalculate`). В `DiscountReleaseIUD.AddItem` при добавлении набора **задним числом между двумя существующими** новому набору ставится `finishDate = @startDate` (собственное начало) вместо `@date` (начало следующего набора). Набор получается нулевой длины и не применяется ни к одной дате, а предыдущему набору конец уже сдвинут — образуется период без скидки. Пример: A 01.01.2024–01.01.2026, B 01.01.2026–открыт; добавили C с датой 01.06.2025 → A до 01.06.2025, C 01.06.2025–01.06.2025 (пустой), период 01.06–31.12.2025 остаётся без скидки. Добавление в конец истории (последний набор) работает верно. Родственное: `UpdateItem` при смене даты принятия не пересчитывает `finishDate` соседей (прочитано в коде, на данных не проверялось).
**Что сделано в ветке клонирования (7304851):** ветка `Clone` считает конец верно (начало следующего набора, предыдущему конец = начало нового, одна транзакция) и отклоняет дубль даты принятия (`DiscountReleaseStartDateExists`). `AddItem`/`UpdateItem` НЕ тронуты.
...
`HeadCompaniesWithActions` — корневое чтение журналов рекламных акций и та самая процедура,
с которой начинались таймауты этого журнала 27.07.2026. Удвоение не
...
**Попытка уже была — и была свёрнута (владелец продукта, 2026-09-21).** При попытке убрать
второе чтение `полезли проблемы`, и решено, что дешевле жить с двойной загрузкой. Чем именно
кончилось, не записано; симптомы стоит выяснить у владельца до следующего захода. Поэтому:
**риск НЕ низкий**, вопреки тому, как это выглядит по коду. Правка трогает общий движок
деревьев, где сброс кэша завязан ещё и на раздачу фильтра дочерним объектам
(`ObjectsIterator.CacheFilterValues`) и на порядок событий обновления узла.

**Когда браться:** только отдельной задачей, не попутно, и начиная с воспроизведения того, что
сломалось в прошлый раз. Проверять по логу: после правки на переключение вида в журнале акций
должна приходиться одна `Actions1`, а не две — и при этом должны остаться рабочими обновление
узла, применение отбора и удаление.

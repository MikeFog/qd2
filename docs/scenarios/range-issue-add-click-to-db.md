...
**Важный инвариант (проверено по `AddRangeIssues.sql`):** `@rollerID` — один скалярный параметр
на весь вызов процедуры, курсор по станциям (`cur_massmedias`) переиспользует его в каждой
итерации `EXEC dbo.IssueIUD @rollerID = @rollerID, ...`. То есть **один клик = один и тот же
ролик атомарно на всех станциях акции одновременно** — разных роликов на разных станциях в одном
слоте в этой архитектуре в принципе не бывает (см. риск #1 — весь курсор в одной транзакции,
частичного состояния по станциям не возникает). Следствие: `AddedIssues` (in-memory, см. риск #4)
безопасно использовать как источник "какой ролик стоит в этом слоте", несмотря на то что там нет
`massmediaID` и хранится только одна "представительная" строка на слот (`Action.BuildAddedIssuesTable`
берёт пересечение слотов по всем кампаниям акции + минимальное количество) — ролик в этой строке
гарантированно совпадает с роликом на любой другой станции того же слота, поэтому потеря привязки
к станции не теряет сам номер ролика. Единственная дыра: если выпуск на одной станции удалили в
обход веерного механизма (напрямую через обычный `CampaignForm`/`RollerIssuesGrid3` этой станции),
слот перестаёт быть общим для всех кампаний — `BuildCommonSlots` тогда просто уберёт его из
`AddedIssues` целиком (безопасный отказ: номер пропадёт, а не станет неверным).

---

## Выбор кампаний и «частичные» слоты

Веер работает не со всей акцией, а с подмножеством её **линейных** кампаний: чек-лист
`grdCampaigns` внизу левой колонки `EditIssuesForm`, применяется кнопкой «Обновить»
(`EditIssuesForm.RefreshGrid` override). Выбор уезжает в SQL параметром `@campaignIDs`
(CSV campaignID; NULL = все линейные кампании акции) в `TariffWindowWithRange`,
`AddRangeIssues`, `MasterIssueDelete`, `RangeSlotIssues`.

Фильтр по `campaignID`, а не по `massmediaID`: по `UIX_Campaign`
(`actionID + massmediaID + campaignTypeID + paymentTypeID + agencyID`) на одной радиостанции
в акции может идти несколько линейных кампаний.

**Цвет текста ячейки** (`TariffWithRangeGrid.MarkCells`, порядок проверок — приоритет):

| Цвет | Значение |
|---|---|
| Синий | Слот есть в `AddedIssues` — выпуск во **всех** выбранных кампаниях |
| Красный | `HasIssuesThisAction` — выпуск текущей акции есть, но **не во всех** выбранных кампаниях |
| Бирюзовый | Выпуски этой же фирмы в **других** акциях, у всех выбранных станций |
| Оранжевый | То же, но только у части станций |

**Частичный (красный) слот** в `AddedIssues` не попадает — там пересечение слотов по кампаниям
(`Action.BuildCommonSlots`). Возникает, например, когда кампании акции идут разными датами или
выпуски правили вне веера. Его содержимое читается из базы процедурой `RangeSlotIssues` и
группируется в C# по паре «ролик + позиция» (`TariffWithRangeGrid.SlotIssueGroup`).

Операции над частичным слотом идут **по тем кампаниям, где выпуск фактически есть**:
`DeleteSlotIssueGroup` / `AddSlotIssueGroup` подставляют в `@campaignIDs` состав группы, а не
весь выбор галочек. Это принципиально: `AddRangeIssues` ставит выпуск во **все** переданные
кампании, и перенос «2 из 5» через общий путь размножил бы рекламу на остальные три.

При переносе частичного слота, если в целевом окне не хватает места хотя бы одной кампании,
`AddRangeIssues` кидает `CannotAddRangeIssues`, C#-транзакция откатывается целиком — перенос
не состоится (решение заказчика: ошибка, а не частичный перенос).

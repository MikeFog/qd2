...
5. **"Пакетная скидка"** — `GetPackageDiscount` (`FrmTemplate3.cs:408`), прямой вызов
   ...
   `PriceCalculatorGrid`.
6. Для веерной (`_massmediaIds.Count > 1`) объёмная скидка выводится как коэффициент по станциям:
   значение, если оно совпадает у всех станций, иначе "разные" (`FormatPerStationValue`, сравнение
   по отформатированной строке). Менеджерская скидка выводится всегда — единый коэффициент на
   акцию. Цена с учётом объёмной скидки — сумма по станциям (`combinedDiscountedSum`), не
   по-станционное значение. См. "Блок Статистика" за полной картиной.
...
| Менеджерская скидка | `lblManagerDiscountValue` | `GetEffectiveManagerDiscount(...)` | тот же коэффициент — единый на акцию (пользователь/период) |
...
#### Объёмная и менеджерская скидка в веере — почему разные стратегии показа

Веерная — несколько одновременно создаваемых кампаний (по одной на СМИ). Обе скидки формально
могут отличаться по станциям, но показываются по-разному:
...
- **Объёмная скидка** — `hlp_CompanyDiscountCalculate` вызывается отдельно на каждое СМИ
  ...
  `btnEstimatePrice_Click`) — у трёх станций веера НА ПРАКТИКЕ часто разные значения (разные
  тарифы/объёмы), поэтому единое число вводило бы в заблуждение. Показывается коэффициент:
  значение, если совпадает у всех станций, иначе "разные" (`FormatPerStationValue`, сравнение по
  отформатированной строке "N2", чтобы незначимые разряды не давали ложное "разные").
- **Цена с учётом объёмной скидки** — в рублях, поэтому в отличие от коэффициента суммируется по
  станциям без потери смысла (`combinedDiscountedSum` / `_baselineDiscountedSum` — сумма
  `Campaign.Price` по станциям), а не сравнивается на равенство.
- **Менеджерская скидка** — `ActionRecalculate` пишет `managerDiscount` отдельно в каждую строку
  `Campaign`, не в `Action`, но на практике всегда одна и та же — один пользователь/период на всю
  акцию. Показывается как единый коэффициент (`GetEffectiveManagerDiscount`, та же логика, что и
  для линейной — первая кампания акции с выпусками, либо гипотетический расчёт для новой).
...
скидку/итог. По дальнейшему фидбеку заказчика прочерки заменены на текущее поведение выше —
дать реальную информацию там, где она осмысленна (менеджерская скидка, сумма по объёмной), и
явно писать "разные" только там, где число по станциям действительно может расходиться
(коэффициент объёмной скидки).
